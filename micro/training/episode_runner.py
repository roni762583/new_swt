...
class EpisodeRunner:
    ...
    def run_episode(
        ...
        for bar in range(360):
            ...
            # Use real MCTS (fixed with simpler non-stochastic version)
            ...
            if trade_result is not None:
                ...
                if trade_result > 0:
                    winning_trades += 1
                self.trades_pnl.append(trade_result)  # Store for R calculation
        ...
        # Calculate expectancy using Peoples Fintech formula
        if hasattr(self, 'trades_pnl') and self.trades_pnl:
            trades_array = np.array(self.trades_pnl[-100:])  # Last 100 trades
            wins = trades_array[trades_array > 0]
            losses = trades_array[trades_array < 0]

            avg_loss = abs(np.mean(losses)) if len(losses) > 0 else 10.0  # R value (default 10 pips)
            expectancy_pips = total_pnl / max(num_trades, 1)  # Current method
            expectancy_R = expectancy_pips / avg_loss  # Peoples Fintech method

            # Use R-based expectancy
            expectancy = expectancy_R

            # Log for debugging
            if num_trades % 10 == 0:  # Log every 10 trades
